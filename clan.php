...
/*//*/	modulehook("clan-enter");
	...
	if ($op==""){
		...
		if ($session['user']['clanrank']>=CLAN_OFFICER){
			...
			rawoutput("<textarea name='clanmotd' cols='50' rows='10' style='width: 100%'>".htmlentities($claninfo['clanmotd'], ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."</textarea><br>");
			...
			if ($claninfo['descauthor']==4294967295){
				...
				rawoutput("<textarea name='clandesc' cols='50' rows='10' style='width: 100%'>".htmlentities($claninfo['clandesc'], ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."</textarea><br>");
			}
...
/*//*/			}
		...
		if ($remove>""){
			...
			db_query($sql);
			debuglog("Player {$session['user']['name']} removed {$whoname} from {$claninfo['clanname']}.", $whoacctid);
		...
		while ($row=db_fetch_assoc($result)){
			...
			if ($session['user']['clanrank']>CLAN_MEMBER){
				...
				if ($row['clanrank']<$session['user']['clanrank']){
					rawoutput("[ <a href='clan.php?op=membership&setrank=".($row['clanrank']+10)."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']."'>$promote</a> | ");
					addnav("","clan.php?op=membership&setrank=".($row['clanrank']+10)."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']);
				}else{
					output_notl("[ %s | ", $promote);
				}
				if ($row['clanrank']<=$session['user']['clanrank'] && $row['clanrank']>CLAN_APPLICANT && $row['login']!=$session['user']['login']){
					rawoutput("<a href='clan.php?op=membership&setrank=".($row['clanrank']-10)."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']."'>$demote</a> | ");
					addnav("","clan.php?op=membership&setrank=".($row['clanrank']-10)."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']);
				}else{
					output_notl("%s | ", $demote);
				}
				if ($row['clanrank'] <= $session['user']['clanrank'] && $row['login']!=$session['user']['login']){
					rawoutput("<a href='clan.php?op=membership&remove=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']."' onClick=\"return confirm('$confirm');\">$remove</a> ]");
					addnav("","clan.php?op=membership&remove=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']);
				}else{
