...
function page_footer(){
	...
	if (getsetting("logdnet",0) && $session['user']['loggedin']){
...
<input type="image" src="images/paypal2.gif" border="0" name="submit" alt="Donate!">
	...
	$footer=str_replace("{pagegen}","Page gen: ".round($gentime,2)."s / ".$dbinfo['queriesthishit']." queries, Ave: ".round($session['user']['gentime']/$session['user']['gentimecount'],2)."s - ".round($session['user']['gentime'],2)."/".round($session['user']['gentimecount'],2)."",$footer);
		...
		while (list($key,$val)=each($session['bufflist'])){
			...
			if ($val['name'] > "" || $session['user']['superuser'] & SU_DEBUG_OUTPUT){
				...
				if ($val['rounds']>=0){
					...
					tlschema("buffs");
					$b = translate_inline("`#%s `7(%s rounds left)`n");
					tlschema();
...
function maillink(){
	...
	if ($row['notseen']>0){
		return sprintf("<a href='mail.php' target='_blank' onClick=\"".popup("mail.php").";return false;\" class='hotmotd'>".translate_inline("Ye Olde Mail: %s new, %s old")."</a>",$row['notseen'],$row['seencount']);
	}else{
		return sprintf("<a href='mail.php' target='_blank' onClick=\"".popup("mail.php").";return false;\" class='motd'>".translate_inline("Ye Olde Mail: %s new, %s old")."</a>",$row['notseen'],$row['seencount']);
	}
}
