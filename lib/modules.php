...
function injectmodule($modulename,$force=false){
	...
	if (file_exists($modulefilename)){
		...
		if (!$force) {
			...
			if ($row['active']){ } else {
				tlschema();
			 	output_notl("`n`3Module `#%s`3 is not active, but was attempted to be injected.`n",$modulename);
				$injected_modules[$force][$modulename]=false;
				return false;
			}
		}
...
function get_module_objpref($type, $objid, $name, $module=false){
	global $mostrecentmodule;
	if ($module === false) $module = $mostrecentmodule;
	$sql = "SELECT * FROM ".db_prefix("module_objprefs")." WHERE modulename='$module' AND objtype='$type' AND setting='".addslashes($name)."' AND objid='$objid' ";
	$result = db_query($sql);
	if (db_num_rows($result)>0){
		$row = db_fetch_assoc($result);
		return $row['value'];
	...
	if(injectmodule($shortname,true)) {
		...
		if (function_exists($fname)){
			...
			if (!isset($moduleinfo['name']) ||
				...
				tlschema("common");
				$ns = translate_inline("Not specified");
				tlschema();
			}
			if (!isset($moduleinfo['name']))
		...
		if (!is_array($moduleinfo) || count($moduleinfo)<2){
			tlschema("common");
			$mf = translate_inline("Missing function");
			tlschema();
			$moduleinfo = array(
...
function module_do_event($type, $module, $allowinactive=false, $baseLink=false)
{
	...
	if(injectmodule($module, $allowinactive)) {
		...
		$fname($type,$baseLink);
		tlschema();
		modulehook("runevent", array("name"=>$module, "type"=>$type, "baselink"=>$baseLink, "get"=>httpallget(), "post"=>httpallpost()));
		//revert nav section after we're done here.
		$navsection = $oldnavsection;
	}
	$mostrecentmodule=$mod;
...
function module_editor_navs($like, $linkprefix)
{
	...
	while($row = db_fetch_assoc($result)) {
		...
		addnav(($row['active'] ? "" : "`)") . $row['formalname'],
...
function module_objpref_edit($type, $module, $id)
{
	...
	if (count($info['prefs-'.$type]) > 0) {
		...
		$sql = "SELECT * FROM " . db_prefix("module_objprefs") . " WHERE modulename='$module' AND objtype='$type' AND objid='$id'";
		$result = db_query($sql);
		while($row = db_fetch_assoc($result)) {
	...
	if (modulename_sanitize($module)!=$module){
		...
		if (injectmodule($module,true)) {
			...
			if (!module_check_requirements($info['requires'])){
				...
				$keys = "|".join(array_keys($info), "|")."|";
				$sql = "INSERT INTO " . db_prefix("modules") . " (modulename,formalname,moduleauthor,active,filename,installdate,installedby,category,infokeys,version,download,description) VALUES ('$mostrecentmodule','".addslashes($info['name'])."','".addslashes($info['author'])."',0,'{$mostrecentmodule}.php','".date("Y-m-d H:i:s")."','$name','".addslashes($info['category'])."','$keys','".addslashes($info['version'])."','".addslashes($info['download'])."', '".addslashes($info['description'])."')";
				db_query($sql);
