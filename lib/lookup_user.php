...
function lookup_user($query=false, $order=false, $fields=false, $where=false){
	...
	if ($query != "") {
		...
		if ($where === false) 
			$sql_where = "WHERE login='$query' OR name='$query'";
		else
	...
	if ($query !== false || $searchresult) {
		if (db_num_rows($searchresult) != 1) {
			...
			if ($where === false) 
				$where="WHERE login LIKE '%$query%' OR acctid LIKE '%$query%' OR name LIKE '%$name_query%' OR emailaddress LIKE '%$query%' OR lastip LIKE '%$query%' OR uniqueid LIKE '%$query%' OR gentimecount LIKE '%$query%' OR level LIKE '%$query%'";
			else
				$sql_where = "WHERE $where"; 
