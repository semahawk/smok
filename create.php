...
if ($op=="val"){
	...
	if (db_num_rows($result)>0) {
		...
		$sql = "UPDATE " . db_prefix("accounts") . " SET emailvalidation='' WHERE emailvalidation='$id' AND emailvalidation!=''";
		db_query($sql);
				...
				if (db_num_rows($result)>0){
					...
					if (db_affected_rows(LINK)<=0){
						...
						if ($emailverification!=""){
							...
							$msg = translate_mail(array("In order to verify your account, you will need to click on the link below.\n\n  http://%s?op=val&id=%s\n\nThanks for playing!",
								($_SERVER['SERVER_NAME'].($_SERVER['SERVER_PORT'] == 80?"":":".$_SERVER['SERVER_PORT']).$_SERVER['SCRIPT_NAME']),
								$emailverification),
								0);
							mail($email,$subj,$msg,"From: ".getsetting("gameadminemail","postmaster@localhost.com"));
