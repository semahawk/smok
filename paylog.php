...
if ($op==""){
	...
	for ($i=0;$i<db_num_rows($result);$i++){
		...
		if ($row['name']>"") {
			...
			rawoutput("-=( <a href='$link' title=\"".htmlentities($info['item_number'], ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\" alt=\"".htmlentities($info['item_number'])."\">[".htmlentities($info['memo'])."]</a> )=-");
			addnav("",$link);
		}
		rawoutput("</td></tr>");
	}
