...
if ($op==""){
	...
	$uri = httpget('u');
	$text = stripslashes(httpget('t'));
	/*
	output("`bUri:`b %s`n",stripslashes($uri));
	output("`bText:`b %s`n",stripslashes($text));
	*/
	...
	rawoutput("$namespace <input name='uri' value=\"".htmlentities(stripslashes($uri), ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\"><br/>");
	rawoutput("$texta<br>");
	rawoutput("<textarea name='text' cols='60' rows='5'>".htmlentities($text, ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."</textarea><br/>");
